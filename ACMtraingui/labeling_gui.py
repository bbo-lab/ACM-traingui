...
class MainWindow(QMainWindow):
    def __init__(self, drive: Path, file_config=None, master=True, parent=None):
        ...
        # Sketch stuff
        self.sketch_fig = Figure()
        self.sketch_canvas = FigureCanvasQTAgg(self.sketch_fig)
        self.sketch_text = None
        self.sketchZoom_labels = []
        self.sketch_labels = []
        self.sketch_zoom_circle = None
        self.sketch_zoom_point = None

        # Sketch zoom stuff
        self.ax_sketch_zoom = None
        ...
        if not (self.cfg['list_fastLabelingMode']):
            self.self.controls['general']['list_fast_labeling_mode'].setCurrentIndex(self.cfg['cam'])
        ...
        if self.master:
            if os.path.isfile(labels_file):
                ...
                if self.controls['status']['label3d_select']:
                    if self.controls['general']['list_labels3d'].currentItem().text() in self.labels2d.keys():
                        self.selectedLabel2d = \
                            np.copy(self.labels2d[self.controls['general']['list_labels3d'].currentItem().text()])
            else:
            ...
            if os.path.isfile(labels_file):
                self.labelsAreLoaded = True
                self.labels2d_all = np.load(labels_file, self.labels2d_all, allow_pickle=True)[
                    'arr_0'].item()
                if self.get_pose_idx() in self.labels2d_all.keys():
                    self.labels2d = copy.deepcopy(self.labels2d_all[self.get_pose_idx()])
                    if self.controls['status']['label3d_select']:
    ...
    def load_model(self, model_file: typing.Optional[Path] = None):
        ...
        if model_file.is_file():
            ...
            if 'labels3d' in self.model:
                ...
                # FIXME: comment when you want to label origin/coord, uncomment if you want to actually label something:
    ...
    def set_layout(self):
        ...
        layout_grid.addWidget(self.frame_views2d, 0, 0, 2, 4)
        ...
        layout_grid.addWidget(self.frame_views3d, 0, 4)
        ...
        layout_grid.setColumnStretch(4, 1)
        layout_grid.addWidget(self.frame_controls, 1, 4)
        # add to grid
    ...
    def plot2d_update(self):
        if self.controls['status']['button_fastLabelingMode']:
            self.plot2d_draw_labels(self.ax2d[self.i_cam], self.i_cam)
            self.fig2d[self.i_cam].canvas.draw()
        else:
            for i_cam in range(len(self.cameras)):
                self.plot2d_draw_labels(self.ax2d[i_cam], i_cam)
    ...
    def plot2d_plot_single_image_ini(self, ax, i_cam):
        ...
        self.imgs[i_cam] = reader.get_data(self.pose_idx)
    ...
    def plot2d_draw_normal_ini(self):
        for i in reversed(range(self.views2d_layoutGrid.count())):
            widget_to_remove = self.views2d_layoutGrid.itemAt(i).widget()
            self.views2d_layoutGrid.removeWidget(widget_to_remove)
        ...
        for i_cam in range(len(self.cameras)):
            ...
            self.ax2d.append(ax)
            self.h_imgs.append([])
            self.plot2d_plot_single_image_ini(self.ax2d[-1], i_cam)
    ...
    def plot2d_draw_normal(self):
        ...
        for i_cam in range(len(self.cameras)):
            self.plot2d_plot_single_image(self.ax2d[i_cam], i_cam)
    ...
    def plot2d_draw_fast_ini(self):
        ...
        self.plot2d_plot_single_image_ini(self.ax2d[self.i_cam], self.i_cam)
    ...
    def plot2d_draw_fast(self):
        ...
        self.plot2d_plot_single_image(self.ax2d[self.i_cam], self.i_cam)
            ...
            if ax is not None:
                ...
                if event.button == 1:
                    ...
                    if (x is not None) and (y is not None):
                        ...
                        if not (self.controls['general']['list_labels3d'].currentItem().text() in self.labels2d.keys()):
                            self.labels2d[self.controls['general']['list_labels3d'].currentItem().text()] = \
                                np.full((len(self.cameras), 2), np.nan, dtype=np.float64)
                        self.labels2d[self.controls['general']['list_labels3d'].currentItem().text()][i_cam] = \
                    ...
                    if self.controls['general']['list_labels3d'].currentItem().text() in self.labels2d.keys():
                        self.labels2d[self.controls['general']['list_labels3d'].currentItem().text()][i_cam] = np.array(
                            [np.nan, np.nan],
                            dtype=np.float64)
                        if np.all(np.isnan(
                                self.labels2d[self.controls['general']['list_labels3d'].currentItem().text()])):
                            del (self.labels2d[self.controls['general']['list_labels3d'].currentItem().text()])
                    if self.get_pose_idx() in self.labels2d_all.keys():
                        if self.controls['general']['list_labels3d'].currentItem().text() in \
                                self.labels2d_all[self.get_pose_idx()].keys():
                            self.labels2d_all[self.get_pose_idx()][
                                self.controls['general']['list_labels3d'].currentItem().text()][
                                i_cam] = np.array(
    ...
    def plot3d_draw(self):
        for i in reversed(range(self.views3d_layoutGrid.count())):
            widget_to_remove = self.views3d_layoutGrid.itemAt(i).widget()
            self.views3d_layoutGrid.removeWidget(widget_to_remove)
            widget_to_remove.setParent(None)
        ...
        if self.modelIsLoaded:
            ...
            self.ax3d.add_collection3d(surf_model3d)

            # self.ax3d.set_aspect('equal')

            self.ax3d.set_xlim([v_center[0] - self.dxyz_lim,
                                v_center[0] + self.dxyz_lim])
            self.ax3d.set_ylim([v_center[1] - self.dxyz_lim,
                                v_center[1] + self.dxyz_lim])
            self.ax3d.set_zlim([v_center[2] - self.dxyz_lim,
    ...
    def sketch_draw(self):
        ...
        ax_sketch = self.sketch_fig.add_axes([ax_sketch_left,
                                              ax_sketch_bottom,
                                              ax_sketch_width,
                                              ax_sketch_height])
        ax_sketch.clear()
        ...
        ax_sketch.set_title('Full:',
                            ha='center', va='center',
                            zorder=0)
        self.sketch_point = ax_sketch.plot([np.nan], [np.nan],
                                           color='darkgreen',
                                           marker='.',
                                           markersize=2,
                                           alpha=1.0,
                                           zorder=2)
        self.sketch_circle = ax_sketch.plot([np.nan], [np.nan],
                                            color='darkgreen',
                                            marker='o',
                                            markersize=20,
                                            markeredgewidth=2,
        ...
        self.ax_sketch_zoom = self.sketch_fig.add_axes([ax_sketch_zoom_left,
                                                        ax_sketch_zoom_bottom,
                                                        ax_sketch_zoom_width,
                                                        ax_sketch_zoom_height])
        self.ax_sketch_zoom.imshow(sketch)
        self.ax_sketch_zoom.set_xlabel('')
        self.ax_sketch_zoom.set_ylabel('')
        self.ax_sketch_zoom.set_xticks(list())
        self.ax_sketch_zoom.set_yticks(list())
        self.ax_sketch_zoom.set_xticklabels(list())
        self.ax_sketch_zoom.set_yticklabels(list())
        self.ax_sketch_zoom.set_title('Zoom:',
                                      ha='center', va='center',
                                      zorder=0)
        self.ax_sketch_zoom.grid(False)
        self.sketch_zoom_point = self.ax_sketch_zoom.plot([np.nan], [np.nan],
                                                          color='darkgreen',
                                                          marker='.',
                                                          markersize=4,
                                                          alpha=1.0,
                                                          zorder=2)
        self.sketch_zoom_circle = self.ax_sketch_zoom.plot([np.nan], [np.nan],
                                                           color='darkgreen',
                                                           marker='o',
                                                           markersize=40,
                                                           markeredgewidth=4,
                                                           ...
                                                           alpha=2 / 3,
                                                           zorder=2)
        self.ax_sketch_zoom.set_xlim(
            [np.shape(sketch)[1] / 2 - self.sketch_zoom_dx, np.shape(sketch)[1] / 2 + self.sketch_zoom_dx])
        self.ax_sketch_zoom.set_ylim(
            [np.shape(sketch)[0] / 2 - self.sketch_zoom_dy, np.shape(sketch)[0] / 2 + self.sketch_zoom_dy])
        self.ax_sketch_zoom.invert_yaxis()
        # text
        self.sketch_text = self.sketch_fig.text(ax_sketch_zoom_left + ax_sketch_zoom_width / 2,
        ...
        for label_name, label_location in self.get_sketch_labels().items():
            ...
            sketch_zoom_labels = self.ax_sketch_zoom.plot([label_location[0]],
                                                          [label_location[1]],
                                                          marker='o',
                                                          color=color,
                                                          markersize=5,
                                                          zorder=1)
            self.sketchZoom_labels.append(sketch_zoom_labels[0])
        try:
            ...
            if not self.controls['status']['label3d_select']:
                ...
                self.controls['general']['list_labels3d'].setCurrentRow(sorted_index)
                self.list_labels3d_select()
        except:
            pass

        self.sketch_update()
        self.views3d_layoutGrid.addWidget(self.sketch_canvas)

    def sketch_update(self):
        if self.controls['status']['label3d_select']:
            label_coordinates = self.get_sketch_label_coordinates()
            selected_label_name = self.controls['general']['list_labels3d'].currentItem().text()
            ...
            with warnings.catch_warnings():
                ...
                if not sellabelerr.size == 0 and not labelerr.size == 0:
                    ...
                    self.sketch_text.set(text=f'Label {(label_index + 1):02d}:\n{selected_label_name}')
            ...
            # zoom
            self.sketch_zoom_point[0].set_data([x], [y])
            self.sketch_zoom_circle[0].set_data([x], [y])
            self.ax_sketch_zoom.set_xlim([x - self.sketch_zoom_dx, x + self.sketch_zoom_dx])
            self.ax_sketch_zoom.set_ylim([y - self.sketch_zoom_dy, y + self.sketch_zoom_dy])
            self.ax_sketch_zoom.invert_yaxis()
    ...
    def set_controls(self):
        ...
        controls['general']['button_load_recording'] = button_load_recording
        col = col + 1
        ...
        controls['general']['button_load_model'] = button_load_model
        col = col + 1
        ...
        controls['general']['button_load_labels'] = button_load_labels
        row = row + 1
        col = 0
        ...
        controls['general']['button_load_calibration'] = button_load_calibration
        col = col + 1
        ...
        controls['general']['button_save_model'] = button_save_model
        col = col + 1
        ...
        controls['general']['button_save_labels'] = button_save_labels
        row = row + 1
        col = 0
        ...
        controls['general']['button_load_origin'] = button_load_origin
        col = col + 1
        ...
        controls['general']['button_load_sketch'] = button_load_sketch
        col = col + 1
        ...
        controls['general']['button_sketch_mode'] = button_sketch_mode
        ...
        controls['general']['button_fast_labeling_mode'] = button_fast_labeling_mode
        ...
        controls['general']['list_fast_labeling_mode'] = list_fast_labeling_mode
        row = row + 2
        col = 0
        ...
        controls['general']['button_centric_view_mode'] = button_centric_view_mode
        ...
        controls['general']['label_dx'] = label_dx
        col = col + 1

        label_dy = QLabel()
        label_dy.setText('dy:')
        controls_layout_grid.addWidget(label_dy, row, col)
        controls['general']['label_dy'] = label_dy
        row = row + 1
        col = 1
        ...
        controls['general']['field_dx'] = field_dx
        col = col + 1
        ...
        controls['general']['field_dy'] = field_dy
        row = row + 1
        col = 0
        ...
        controls['general']['button_reprojection_mode'] = button_reprojection_mode
        ...
        controls['general']['label_vmin'] = label_vmin
        col = col + 1

        label_vmax = QLabel()
        label_vmax.setText('vmax:')
        controls_layout_grid.addWidget(label_vmax, row, col)
        controls['general']['label_vmax'] = label_vmax
        row = row + 1
        col = 0
        ...
        controls['general']['field_vmin'] = field_vmin
        col = col + 1
        ...
        controls['general']['field_vmax'] = field_vmax
        row = row + 1
        col = 0
        ...
        controls['general']['field_labels3d'] = field_labels3d
        col = col + 1
        ...
        controls['general']['list_labels3d'] = list_labels3d
        row = row + 1
        col = 0
        ...
        controls['general']['button_insert'] = button_insert
        row = row + 1
        ...
        controls['general']['button_remove'] = button_remove
        row = row + 1
        ...
        controls['general']['button_label3d'] = button_label3d
        controls['status']['button_label3d'] = False
        col = col + 1
        ...
        controls['general']['button_previous_label'] = button_previous_label
        col = col + 1
        ...
        controls['general']['button_next_label'] = button_next_label
        row = row + 1
        col = 0
        ...
        controls['general']['button_up'] = button_up
        col = col + 1
        ...
        controls['general']['button_right'] = button_right
        col = col + 1

        label_dxyz = QLabel()
        label_dxyz.setText('dxyz:')
        controls_layout_grid.addWidget(label_dxyz, row, col)
        controls['general']['label_dxyz'] = label_dxyz
        row = row + 1
        col = 0
        ...
        controls['general']['button_down'] = button_down
        col = col + 1
        ...
        controls['general']['button_left'] = button_left
        col = col + 1
        ...
        controls['general']['field_dxyz'] = field_dxyz
        row = row + 1
        col = 0
        ...
        controls['general']['button_previous'] = button_previous
        col = col + 1
        ...
        controls['general']['button_next'] = button_next
        col = col + 1
        ...
        controls['general']['button_home'] = button_home
        row = row + 1
        col = 0
        ...
        controls['general']['label_current_pose'] = label_current_pose
        col = col + 1
        ...
        controls['general']['field_current_pose'] = field_current_pose
        col = col + 1
    ...
    def button_load_recording_press(self):
        ...
        if len(rec_file_names_unsorted) > 0:
            ...
            self.controls['general']['button_load_recording'].setStyleSheet("background-color: green;")
            print('Loaded recording:')
            for i_rec in rec_file_names:
                print(i_rec)

            self.controls['general']['list_fast_labeling_mode'].currentIndexChanged.disconnect()
            self.controls['general']['list_fast_labeling_mode'].clear()
            self.controls['general']['list_fast_labeling_mode'].addItems([str(i) for i in range(len(self.cameras))])
            self.controls['general']['list_fast_labeling_mode'].currentIndexChanged.connect(
                self.list_fast_labeling_mode_change)

        self.controls['general']['button_load_recording'].clearFocus()
    ...
    def button_load_calibration_press(self):
        ...
        if file_name:
            ...
            self.controls['general']['button_load_calibration'].setStyleSheet("background-color: green;")
            print('Loaded calibration ({:s})'.format(file_name))
        self.controls['general']['button_load_calibration'].clearFocus()
    ...
    def button_load_origin_press(self):
        ...
        self.controls['general']['button_load_origin'].clearFocus()
    ...
    def button_load_model_press(self):
        ...
        if file_name:
            ...
            self.controls['general']['list_labels3d'].clear()
            self.controls['general']['list_labels3d'].addItems(sorted(list(self.labels3d.keys())))
            ...
            self.plot3d_draw()
            self.controls['general']['button_load_model'].setStyleSheet("background-color: green;")
            print('Loaded model ({:s})'.format(file_name))
        self.controls['general']['button_load_model'].clearFocus()
    ...
    def button_save_model_press(self):
        ...
        self.controls['general']['button_save_model'].clearFocus()
    ...
    def button_load_labels_press(self):
        ...
        if file_name:
            ...
            self.labelsAreLoaded = True
            self.controls['general']['button_load_labels'].setStyleSheet("background-color: green;")
            print('Loaded labels ({:s})'.format(file_name))
        self.controls['general']['button_load_labels'].clearFocus()
    ...
    def button_save_labels_press(self):
        ...
        self.controls['general']['button_save_labels'].clearFocus()
    ...
    def button_load_sketch_press(self):
        ...
        if self.sketchIsLoaded:
            self.controls['general']['button_load_sketch'].setStyleSheet("background-color: green;")
            print('Loaded sketch ({:s})'.format(file_name_sketch))

        self.controls['general']['button_load_sketch'].clearFocus()

    def button_sketch_mode_press(self):
        if self.controls['general']['button_fast_labeling_mode']:
            if self.modelIsLoaded:
                if self.controls['status']['button_sketchMode']:
                    self.controls['general']['button_sketch_mode'].setStyleSheet("background-color: darkred;")
                    self.controls['status']['button_sketchMode'] = not self.controls['status']['button_sketchMode']
                    self.plot3d_draw()
                    self.sketch_fig.canvas.mpl_disconnect(self.cidSketch)
                else:
                    print('WARNING: Model needs to be loaded first')
            elif self.sketchIsLoaded:
                if True:
                    self.controls['general']['button_sketch_mode'].setStyleSheet("background-color: green;")
                    self.controls['status']['button_sketchMode'] = not self.controls['status']['button_sketchMode']
                    self.sketch_draw()
                    self.cidSketch = self.sketch_canvas.mpl_connect('button_press_event',
        ...
        self.controls['general']['button_sketch_mode'].clearFocus()

    def button_insert_press(self):
        if ((not (np.any(np.isnan(self.clickedLabel3d)))) &
                (self.controls['general']['field_labels3d'].text() != '')):
            if self.controls['general']['field_labels3d'].text() in self.labels3d:
                print('WARNING: Label already exists')
            else:
                self.labels3d[self.controls['general']['field_labels3d'].text()] = copy.deepcopy(self.clickedLabel3d)
                self.controls['general']['list_labels3d'].addItems([self.controls['general']['field_labels3d'].text()])
                self.clickedLabel3d = np.array([np.nan, np.nan, np.nan], dtype=np.float64)
                self.controls['general']['field_labels3d'].setText(str(''))
                self.plot3d_update()
        self.controls['general']['button_insert'].clearFocus()
        self.controls['general']['field_labels3d'].clearFocus()

    def button_remove_press(self):
        if self.controls['general']['list_labels3d'].currentItem() is not None:
            self.controls['status']['label3d_select'] = False
            self.selectedLabel3d = np.array([np.nan, np.nan, np.nan], dtype=np.float64)
            self.selectedLabel2d = np.full((len(self.cameras), 2), np.nan, dtype=np.float64)
            self.clickedLabel2d = np.array([np.nan, np.nan], dtype=np.float64)
            del (self.labels3d[self.controls['general']['list_labels3d'].currentItem().text()])
            self.controls['general']['list_labels3d'].takeItem(self.controls['general']['list_labels3d'].currentRow())
            self.plot2d_update()
            self.plot3d_update()
        self.controls['general']['button_remove'].clearFocus()
    ...
    def list_labels3d_select(self):
        ...
        self.selectedLabel3d = copy.deepcopy(
            self.labels3d[self.controls['general']['list_labels3d'].currentItem().text()])
        if self.controls['general']['list_labels3d'].currentItem().text() in self.labels2d.keys():
            self.selectedLabel2d = np.copy(
                self.labels2d[self.controls['general']['list_labels3d'].currentItem().text()])
        else:
        ...
        if self.controls['status']['button_sketchMode']:
            ...
            self.plot3d_update()
        self.controls['general']['list_labels3d'].clearFocus()
    ...
    def button_fast_labeling_mode_press(self):
        if self.recordingIsLoaded:
            if self.controls['status']['button_fastLabelingMode']:
                if self.controls['status']['button_centricViewMode']:
                    self.button_centric_view_mode_press()
                self.controls['general']['button_fast_labeling_mode'].setStyleSheet("background-color: darkred;")
                self.controls['status']['button_fastLabelingMode'] = not self.controls['status'][
                    'button_fastLabelingMode']
                self.plot2d_draw_normal_ini()
            else:
                self.controls['general']['button_fast_labeling_mode'].setStyleSheet("background-color: green;")
                self.controls['status']['button_fastLabelingMode'] = not self.controls['status'][
                    'button_fastLabelingMode']
                self.plot2d_draw_fast_ini()
            if self.controls['status']['button_sketchMode']:
        ...
        self.controls['general']['button_fast_labeling_mode'].clearFocus()

    # getting camera selection by directly accesing list_fastLabelingMode
    def list_fast_labeling_mode_change(self):
        self.i_cam = self.controls['general']['list_fast_labeling_mode'].currentIndex()
        x_res = self.get_x_res()
        y_res = self.get_y_res()
        ...
        if self.controls['status']['button_sketchMode']:
            self.sketch_update()
        self.controls['general']['list_fast_labeling_mode'].clearFocus()

    def button_centric_view_mode_press(self):
        if self.controls['status']['button_fastLabelingMode']:
            if not (self.controls['general']['list_labels3d'].currentItem() is None):
                ...
                if not self.controls['status']['button_centricViewMode']:
                    self.controls['general']['button_centric_view_mode'].setStyleSheet("background-color: darkred;")
                else:
                    self.controls['general']['button_centric_view_mode'].setStyleSheet("background-color: green;")
            else:
        ...
        self.controls['general']['button_centric_view_mode'].clearFocus()
    ...
    def button_reprojection_mode_press(self):
        if self.calibrationIsLoaded:
            ...
            if self.controls['status']['button_reprojectionMode']:
                self.controls['general']['button_reprojection_mode'].setStyleSheet("background-color: green;")
            else:
                self.controls['general']['button_reprojection_mode'].setStyleSheet("background-color: darkred;")
            self.plot2d_update()
        else:
            print('WARNING: Calibration needs to be loaded first')
        self.controls['general']['button_reprojection_mode'].clearFocus()

    def field_dx_change(self):
        try:
            int(self.controls['general']['field_dx'].text())
            field_input_is_correct = True
        except ValueError:
            field_input_is_correct = False
        if field_input_is_correct:
            self.dx = int(np.max([8, int(self.controls['general']['field_dx'].text())]))
        self.controls['general']['field_dx'].setText(str(self.dx))
        if self.controls['status']['button_fastLabelingMode']:
            self.plot2d_draw_fast()
        self.controls['general']['field_dx'].clearFocus()

    def field_dy_change(self):
        try:
            int(self.controls['general']['field_dy'].text())
            field_input_is_correct = True
        except ValueError:
            field_input_is_correct = False
        if field_input_is_correct:
            self.dy = int(np.max([8, int(self.controls['general']['field_dy'].text())]))
        self.controls['general']['field_dy'].setText(str(self.dy))
        if self.controls['status']['button_fastLabelingMode']:
            self.plot2d_draw_fast()
        self.controls['general']['field_dy'].clearFocus()

    def field_vmin_change(self):
        try:
            int(self.controls['general']['field_vmin'].text())
            field_input_is_correct = True
        except ValueError:
            field_input_is_correct = False
        if field_input_is_correct:
            self.vmin = int(self.controls['general']['field_vmin'].text())
        ...
        if self.controls['status']['button_fastLabelingMode']:
            ...
            for i_cam in range(len(self.cameras)):
                self.h_imgs[i_cam].set_clim(self.vmin, self.vmax)
                self.fig2d[i_cam].canvas.draw()
        self.controls['general']['field_vmin'].setText(str(self.vmin))
        self.controls['general']['field_vmin'].clearFocus()

    def field_vmax_change(self):
        try:
            int(self.controls['general']['field_vmax'].text())
            field_input_is_correct = True
        except ValueError:
            field_input_is_correct = False
        if field_input_is_correct:
            self.vmax = int(self.controls['general']['field_vmax'].text())
        ...
        if self.controls['status']['button_fastLabelingMode']:
            ...
            for i_cam in range(len(self.cameras)):
                self.h_imgs[i_cam].set_clim(self.vmin, self.vmax)
                self.fig2d[i_cam].canvas.draw()
        self.controls['general']['field_vmax'].setText(str(self.vmax))
        self.controls['general']['field_vmax'].clearFocus()
    ...
    def button_home_press(self):
        ...
        if self.recordingIsLoaded:
            ...
            for i_cam in range(len(self.cameras)):
                self.xLim_prev[i_cam] = np.array([0.0, x_res[i_cam] - 1], dtype=np.float64)
                self.yLim_prev[i_cam] = np.array([0.0, y_res[i_cam] - 1], dtype=np.float64)
            if self.controls['status']['button_fastLabelingMode']:
                self.plot2d_draw_fast()
            else:
        ...
        if self.modelIsLoaded:
            ...
            self.ax3d.set_zlim([v_center[2] - self.dxyz_lim,
                                v_center[2] + self.dxyz_lim])
            self.canvas3d.draw()
        self.controls['general']['button_home'].clearFocus()
    ...
    def button_zoom_press(self, tostate=None):
        ...
        if self.recordingIsLoaded:
            if not self.controls['status']['toolbars_zoom']:
                self.controls['general']['button_zoom'].setStyleSheet("background-color: green;")
            else:
                self.controls['general']['button_zoom'].setStyleSheet("background-color: darkred;")
            if self.controls['status']['toolbars_pan']:
                self.button_pan_press()
            for i in self.toolbars:
                i.zoom()
    ...
    def button_pan_press(self):
        if self.recordingIsLoaded:
            if not self.controls['status']['toolbars_pan']:
                self.controls['general']['button_pan'].setStyleSheet("background-color: green;")
            else:
                self.controls['general']['button_pan'].setStyleSheet("background-color: darkred;")
            if self.controls['status']['toolbars_zoom']:
                self.button_zoom_press()
            for i in self.toolbars:
                i.pan()
        ...
        if self.modelIsLoaded:
            if not self.controls['status']['button_sketchMode']:
                if not self.controls['status']['button_label3d']:
                    ...
                    self.selectedLabel2d = np.full((len(self.cameras), 2), np.nan, dtype=np.float64)
                    self.selectedLabel3d = np.array([np.nan, np.nan, np.nan], dtype=np.float64)
                    self.controls['general']['list_labels3d'].setCurrentItem(None)
                    self.controls['general']['button_label3d'].setStyleSheet("background-color: green;")
                    ...
                    self.controls['general']['button_label3d'].setStyleSheet("background-color: darkred;")
                    self.ax3d.mouse_init()
            ...
            print('WARNING: Model needs to be loaded first')
        self.controls['general']['button_label3d'].clearFocus()
    ...
    def button_up_press(self):
        ...
        self.controls['general']['button_up'].clearFocus()

    def button_down_press(self):
        move_direc = -2
        self.plot3d_move_center(move_direc)
        self.controls['general']['button_down'].clearFocus()

    def button_left_press(self):
        move_direc = +1
        self.plot3d_move_center(move_direc)
        self.controls['general']['button_left'].clearFocus()

    def button_right_press(self):
        move_direc = -1
        self.plot3d_move_center(move_direc)
        self.controls['general']['button_right'].clearFocus()

    def field_dxyz_change(self):
        try:
            float(self.controls['general']['field_dxyz'].text())
            field_input_is_correct = True
        except ValueError:
            field_input_is_correct = False
        if field_input_is_correct:
            self.dxyz = float(self.controls['general']['field_dxyz'].text())
        self.controls['general']['field_dxyz'].setText(str(self.dxyz))
        self.controls['general']['field_dxyz'].clearFocus()
    ...
    def button_next_label_press(self):
        if self.controls['status']['label3d_select']:
            ...
            self.controls['general']['list_labels3d'].setCurrentRow(sorted_index)
            self.list_labels3d_select()
        self.controls['general']['button_next_label'].clearFocus()
    ...
    def button_previous_label_press(self):
        if self.controls['status']['label3d_select']:
            ...
            self.controls['general']['list_labels3d'].setCurrentRow(sorted_index)
            self.list_labels3d_select()
        self.controls['general']['button_previous_label'].clearFocus()
    ...
    def button_next_press(self):
        ...
        self.controls['general']['button_next'].clearFocus()
    ...
    def button_previous_press(self):
        ...
        self.set_pose_idx(self.get_pose_idx() - self.dFrame)
        self.plot2d_change_frame()
        self.controls['general']['button_previous'].clearFocus()

    def field_current_pose_change(self):
        try:
            int(self.controls['general']['field_current_pose'].text())
            field_input_is_correct = True
        except ValueError:
            field_input_is_correct = False
        if field_input_is_correct:
            ...
            self.set_pose_idx(self.controls['general']['field_current_pose'].text())
            self.plot2d_change_frame()
        self.controls['general']['field_current_pose'].setText(str(self.get_pose_idx()))
        if self.controls['status']['button_sketchMode']:
            first_label_name = self.labels3d_sequence[0]
            sorted_index = sorted(list(self.labels3d.keys())).index(first_label_name)
            self.controls['general']['list_labels3d'].setCurrentRow(sorted_index)
            self.list_labels3d_select()
            self.sketch_update()
        self.controls['general']['field_current_pose'].clearFocus()

    def field_d_frame_change(self):
        try:
            int(self.controls['general']['field_d_frame'].text())
            field_input_is_correct = True
        except ValueError:
            field_input_is_correct = False
        if field_input_is_correct:
            self.dFrame = int(np.max([1, int(self.controls['general']['field_d_frame'].text())]))
            x_res = self.get_x_res()
            y_res = self.get_y_res()
            ...
            for i_cam in range(len(self.cameras)):
                self.xLim_prev[i_cam] = np.array([0.0, x_res[i_cam] - 1], dtype=np.float64)
                self.yLim_prev[i_cam] = np.array([0.0, y_res[i_cam] - 1], dtype=np.float64)
        self.controls['general']['field_d_frame'].setText(str(self.dFrame))
        self.controls['general']['field_d_frame'].clearFocus()

    def plot2d_change_frame(self):
        if self.controls['status']['button_fastLabelingMode']:
            self.cameras[self.i_cam]['x_lim_prev'] = self.ax2d[self.i_cam].get_xlim()
            self.cameras[self.i_cam]['y_lim_prev'] = self.ax2d[self.i_cam].get_ylim()  # [::-1]
        else:
            for i_cam in range(len(self.cameras)):
        ...
        if self.get_pose_idx() in self.labels2d_all.keys():
            ...
            self.labels2d = dict()
        if self.controls['status']['label3d_select']:
            label_name = self.controls['general']['list_labels3d'].currentItem().text()
    ...
    def keyPressEvent(self, event):
        if not (event.isAutoRepeat()):
            if self.cfg['button_next'] and event.key() == Qt.Key_D:
                ...
                self.controls['general']['field_vmax'].setText(str(int(self.vmax * 0.8)))
                ...
                if len(self.cameras) > 0:
                    self.controls['general']['list_fast_labeling_mode'].setCurrentIndex(0)
            elif event.key() == Qt.Key_2:
                if len(self.cameras) > 1:
                    self.controls['general']['list_fast_labeling_mode'].setCurrentIndex(1)
            elif event.key() == Qt.Key_3:
                if len(self.cameras) > 2:
                    self.controls['general']['list_fast_labeling_mode'].setCurrentIndex(2)
            elif event.key() == Qt.Key_4:
                if len(self.cameras) > 3:
                    self.controls['general']['list_fast_labeling_mode'].setCurrentIndex(3)
            elif event.key() == Qt.Key_5:
                if len(self.cameras) > 4:
                    self.controls['general']['list_fast_labeling_mode'].setCurrentIndex(4)
            elif event.key() == Qt.Key_6:
                if len(self.cameras) > 5:
                    self.controls['general']['list_fast_labeling_mode'].setCurrentIndex(5)
